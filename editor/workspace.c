...
const char *re_workspace_root_path(ReApp *a, const char *id) {
  const cJSON *root = NULL;
  cJSON_ArrayForEach(root, cJSON_GetObjectItemCaseSensitive(a->state, "roots"))
...
static void sessions_ui(ReApp *a, mu_Context *ui) {
  ...
  cJSON_ArrayForEach(session, cJSON_GetObjectItemCaseSensitive(a->state, "sessions")) {
    ...
    char label[1024]; snprintf(label, sizeof(label), "%s · %s", re_string(session, "title"), re_workspace_root_name(a, root));
  ...
  cJSON_ArrayForEach(draft, cJSON_GetObjectItemCaseSensitive(a->state, "drafts")) {
    ...
    char label[2300]; snprintf(label, sizeof(label), "%s · %s", re_workspace_root_name(a, re_string(draft, "rootId")), path);
...
void re_workspace_command(ReApp *a, int command) {
  switch (command) {
...
void re_app_ui(ReApp *a, mu_Context *ui, int width, int height) {
  ...
  if (mu_begin_window_ex(ui, "Toolbar", mu_rect(0, 0, width, RE_METRIC_DESIGN_TOOLBAR_HEIGHT), opts | MU_OPT_NOFRAME)) {
    ...
    struct { const char *label; int icon; } switcher[] = {
      ...
      {"Plugins", RE_ICON_ADD}, {"Shell", RE_ICON_SHELL}, {"Agent", RE_ICON_AGENT} };
    ...
    for (int i = 0; i < views; i++, view_index++) {
      ...
      if (toolbar_cell(&bar, switcher[i].label, switcher[i].icon, opt, i ? 0 : RE_METRIC_DESIGN_GAP_LG)) {
        /* Dispatched by LABEL rather than by index: the previous form read `i == 5` for the agent,
           so inserting a view silently reassigned two buttons. */
        const char *chose = switcher[i].label;
        if (!strcmp(chose, "Tree")) { if (*a->root) re_app_tab(a, RE_TREE, a->root, "", "", "Project"); }
        else if (!strcmp(chose, "Dashboard")) { if (re_app_dashboard(a, a->root) < 0) re_copy(a->status, sizeof(a->status), "Add or select a project first."); }
        else if (!strcmp(chose, "Tasks")) { if (re_app_tracker(a, a->root) < 0) re_copy(a->status, sizeof(a->status), "Add or select a project first."); }
        else if (!strcmp(chose, "Devices")) { if (re_app_devices(a, a->root) < 0) re_copy(a->status, sizeof(a->status), "Add or select a project first."); }
        else if (!strcmp(chose, "Plugins")) { if (re_app_extensions(a, a->root) < 0) re_copy(a->status, sizeof(a->status), "Add or select a project first."); }
        else launch_terminal(a, !strcmp(chose, "Agent"), false);
      }
  ...
  for (int n = 0; n < RE_PANES; n++) {
    ...
    if (mu_begin_window_ex(ui, title, content, content_opts | MU_OPT_NOFRAME)) {
      ...
      if (t->type != RE_TREE && t->type != RE_PLUGIN) { mu_layout_row(ui, 1, (int[]){-1}, RE_METRIC_PANE_ROOT_ROW_HEIGHT); re_ui_label_ex(ui, re_workspace_root_name(a, t->root), RE_UI_MUTED); }
      ...
      else if (t->type == RE_EXTENSIONS) re_extensions_ui(a, ui, index);
...
bool re_app_event(ReApp *a, const SDL_Event *e, ReDraw *draw) {
  ...
  if (a->overlay) {
    ...
    if (e->type == SDL_KEYDOWN && e->key.keysym.sym == SDLK_ESCAPE) {
      if (*a->dropdown) a->dropdown[0] = 0; else re_workspace_overlay_close(a);
      return true;
    }
