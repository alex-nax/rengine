...
| 9 | A pane is **nobody's child session**. Claude Code stamps every process it starts with its own identity (`CLAUDECODE`, `CLAUDE_PID`, `CLAUDE_CODE_CHILD_SESSION`, `CLAUDE_CODE_SESSION_ID`, the messaging socket and token, …), and a host started from inside a pane — `scripts/replace-host.sh` in a consumer, or `editor.sh` from an agent's Bash tool — carries them into every pane it spawns. The CLI reads an inherited `CLAUDE_CODE_CHILD_SESSION` as "nested child" and **saves no transcript**, so the conversation D3 minted cannot be resumed. `shellEnvironment` drops the set from the inherited layer exactly as it drops `NO_COLOR` (D1/D2 rules: explicit override wins, case-insensitive on Windows); preferences such as `CLAUDE_DIFF_TOOL` and `CLAUDE_CODE_*` configuration stay. Found 2026-09-15 on the vtmb-vr host (KI-113); rationale in `orchestrator/server/sessions-client.mjs._llm.json#a-pane-is-nobodys-child-session`. | Recommended; the NO_COLOR incident's shape a second time |
...
| 3c | The host mints and the launcher decides, but **the CLI has the last word**. A person who resumes another conversation from inside the running CLI moves the process without touching any flag, so Claude Code's `SessionStart` hook reports the `session_id` it is actually running back over the same `POST /api/agent-conversation` this table's decision 7 already uses, and rewrites the per-launch identity with it. Decision 3's eager mint is unaffected: it still names the conversation before the first byte of output; it is now a claim the CLI can correct rather than the last word. See [095 *The CLI reports what it runs*](095-project-token.md#the-cli-reports-what-it-runs-2026-09-07). |
...
| 4b | So an agent that names its own conversation is recorded with none **until it says otherwise**. A `-c` or `--fork-session` launch still injects nothing and still reports `conversation: null`, and the pane still refuses `restart_agent` by name — but only until the hook's first report, after which the pane holds the id the CLI minted for itself and is restartable like any other. Nothing is invented in the meantime, which is what decision 4 was protecting. |
...
| `environment.test.mjs`, `agent-spawn-env.test.mjs` | an inherited Claude Code process identity never reaches a pane while `CLAUDE_DIFF_TOOL`/`CLAUDE_EFFORT` do, an explicit override still wins, the drop is case-insensitive on Windows — and the Rust envelope answers the same on the parity fixture (D9, KI-113) |
...
| `report-session.test.mjs` | the launch carries the `--settings` hook; a conversation the CLI reports replaces the launched one in the host record and in the per-launch identity; the hook is silent and exits 0 outside a pane; a `-c` launch becomes known and restartable once the CLI reports; and `workspace_info` follows without a worker restart |
