...
contract 1), 075 (dashboard, contract 2) and 076 (game surface) remain their authorities.
...
4. **Declare the project** in `.rengine/project.json`: `formats` over the project's own CLI with
   the `${file}`/`${entry}` placeholders (contract 1), plus `game` and `dashboard` (contract 2).
...
| The runbook, the wizard and these templates | The choice of formats, dashboard groups, game surface and pin cadence |
...
`.rengine/project.json` skeleton (with the `game` block when the game arguments are supplied and a
dashboard whose one action runs `editor.sh --check`), copying the generic declaration test, and
printing the follow-ups. Existing files are reported and skipped, never overwritten, so the wizard
is safe to re-run on a partially integrated project.

Decisions the wizard must not make: which formats a project registers and which CLI produces them;
what belongs in the dashboard groups; whether the game runs `external` or through the
`sdl2-interpose` surface; how the declaration test is registered with the project's own runner
(ctest, pytest, a make target); the pin bump cadence; and the wording of the CLAUDE/AGENTS line.
The wizard prints each of these as a follow-up instead of guessing.
...
2. `orchestrator/actions/integrate-project.sh` follows the wizard conventions (`re_wizard`,
   ...
   unknown options and missing values with exit 2, and prints its follow-ups on completion.
...
4. `orchestrator/tests/integrate-project.test.mjs` scaffolds a temporary git repository and
   asserts: the declaration exists and satisfies the contract-2 rules structurally; the same
   skeleton at `contract: 1` without `game`/`dashboard` passes this branch's `readDeclaration`
   with no error; `editor.sh --dry-run --rebuild --bootstrap-only` prints the three bootstrap
   commands and no launch line; the copied Python test passes with `--root` on the scaffold;
   existing files are skipped; `--dry-run` writes nothing. A submodule stage runs against a local
   `file://` clone when this git allows it, and asserts the dry-run command list otherwise.
