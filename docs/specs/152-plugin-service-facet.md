...
| 12 | **A `secret` field is masked while it is typed, is pasted into, and opens empty.** The buffer under the field is the real one — editing, backspace and the caret all count real characters — and what reaches the screen is one `*` per character. There is nothing to prefill it with, because nothing on this side ever knew the value: the gesture the page offers is *replace*, never *edit*. A key drawn in full would be a key in every screenshot and in every snapshot a spec writes. **Paste is the gesture this field exists for**: a key is around a hundred characters and always arrives from a clipboard, so an owned textbox reads the clipboard on the platform's paste chord — it did not, which made the one field meant for a pasted credential the one field a credential could not be pasted into. | Recommended; the paste half after the owner hit it |
| 13 | **The plugin routes are answered by the module BOTH servers call, not by either one.** `red_project::serve` exists because the door and the worker both answer what a project declares and neither may forward it. The first implementation put these routes in the worker alone, and the door — which *is* the whole workspace when nothing is behind it — answered 404: the Plugins page loaded empty at the address the desktop actually uses. The door also kept a hand-written copy of that module's route table, which is what let the two drift; it now asks `serve::owns`. | Correction, 2026-09-18 |
