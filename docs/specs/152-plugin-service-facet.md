...
| 12 | **A `secret` field is masked while it is typed, and opens empty.** The buffer under the field is the real one — editing, backspace and the caret all count real characters — and what reaches the screen is one `*` per character. There is nothing to prefill it with, because nothing on this side ever knew the value: the gesture the page offers is *replace*, never *edit*. A key drawn in full would be a key in every screenshot and in every snapshot a spec writes. | Recommended |
