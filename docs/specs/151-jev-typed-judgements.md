...
| 10 | **The strategic comparison was missing**: this project's committed direction is a local model (D68/D73). | **Accepted.** F231 attempts the same classifications with **Gemma 4 E2B QAT**, the pilot model of spec 150 decision 7, through `re_model`'s GBNF grammar — which the pack's ABI already carries, so the comparison needs no new capability. Owner, 2026-09-18: *"for local model - let's use the one intended for pilot agent"*. Two conditions on fairness: the comparison is on **decision quality against outcomes**, never on comparing the two confidence numbers — Jev returns calibrated probabilities and a grammar-constrained general model returns a token, and those are not the same object; and E2B is the **floor** chosen for a phone, so a loss there is re-run at E4B before it is read as "local cannot do this". |
