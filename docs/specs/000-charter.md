...
| D59 | **Android gets an OpenGL ES path beside Vulkan, chosen by what the device offers.** D58 scoped mobile to Vulkan-only, overruling MoltenVK and extending the desktop's per-platform pattern. Measurement on the owner's own phone reversed it: an Adreno 619 on a 2022 driver reports **Vulkan 1.1.128** and its device extension list **lacks `VK_KHR_dynamic_rendering` and `VK_KHR_synchronization2`**, both of which the seam's Vulkan backend is built on — so the companion could not render there at all, while the same build brought the renderer up on a Quest 3 (Adreno 740, Vulkan 1.3.295). F145's premise is *a phone on cellular*, which means mid-range hardware, and all 56 GL entry points the seam loads exist in OpenGL ES 3.x. So the pack's OpenGL backend gains an ES dialect and Android gains an EGL host; Vulkan stays the path where the device offers it. D29's GL-first order remains scoped to the desktop — this is not that order returning, it is one backend the pack already has reaching one more class of device. | Owner, 2026-09-11: "Can we use opengl on this phone then?", amending D58; spec 128 |
| D60 | **PTY sessions belong to the state directory, not to whichever host is running.** Specs 059/060 promise reattachment after a GUI restart and say a replaced service still needs explicit reconnection; today a host replacement ends every retained pane, which is why the owner's session host is never restarted without their say-so. F176 moved the PTYs into their own process, making the lifetime a choice: `red-pty-serve` is long-lived per state directory, found through a `pty.json` descriptor with the same loopback-plus-token discipline `sidecar.json` already uses, and a starting host adopts the live sessions by attaching and snapshotting rather than by handover. A protocol mismatch is refused by name and falls back to ending that service, which is today's behavior. The rejected alternative — a per-host child, preserving today's semantics and reaching a node-free stack one tick sooner — loses to the fact that every remaining J0 row replaces the host to prove itself. | Owner, 2026-09-13, choosing "Per-state-dir service, survives host replacement" for F177; spec 131. |
| D61 | **The workspace store becomes a per-state-directory service too, for the length of the host arc and after it.** Moving a route into red-host moves the state it owns, and `store-client.mjs` spawns its own `red-store-serve` per host process — so a front door serving `/api/tree` or `/api/save` from its own store while the JS backend still held the same state directory would be two in-memory owners of one set of files: stale reads on one side, lost writes on the other (KI-103). red-store takes the shape red-pty already has under D60 — a `store.json` descriptor in the state directory, one long-lived service, every host attaching to it — which is the pattern this repository has now run twice, makes each later row a small route move, and is what the host does anyway once the JS side is gone. The rejected alternative, forwarding every state-touching route until the JS host is deleted whole, keeps one owner trivially and leaves the biggest unknown — whether a Rust host serves the desktop's state correctly — untested until the last commit. | Owner, 2026-09-13, choosing "Per-state-dir store service, both hosts attach"; KI-103, spec 129. |
| D62 | **A pane's record belongs to the same service its process does.** D60 gave the PTY process one owner per state directory, but the pane's *record* — state, title, conversation, task, the handoff gate and whether it has been released — stayed in whichever host spawned it; the service's `meta` blob is written once at spawn and never updated, because it exists so the next host can name what it adopts, not so two hosts can share it. That is KI-103 one level down, and its failure mode is worse than a stale read: a front door answering `/api/input` from its own copy refuses a pane the person is already typing into, with `Handoff is waiting for its native view.` So red-pty gains the update side it lacks — one `describe` that merges into the record and broadcasts it — and every host reads and writes the pane there. `meta` becomes the live record it was already shaped like. The rejected alternatives: moving the session routes and `/events` in one row (one owner trivially, but `/api/state` and `/api/dashboard-run` still call sessions in the backend, so the backend would have to ask its own door to spawn a pane), and leaving the whole session surface until F153–F156 have emptied the backend. | Owner, 2026-09-13, choosing "The record becomes the service's, live"; KI-104, specs 129/131. |
