...
| 2 | Rasterising is a **vendored library, wrapped thinly** — `third_party/nanosvg` (zlib) behind `editor/svg.{c,h}`, exactly as `jpeg.c` wraps pinned stb. Writing a path rasteriser would be rEngine's own work in a solved problem, and the Kohai wordmark alone needs cubic béziers. | Recommended |
