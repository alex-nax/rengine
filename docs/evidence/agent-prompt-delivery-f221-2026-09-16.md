...
Spec 149, over spec 141's rule. Reported from the NOLF workspace as a pane that died:
...
mechanism is exercised and no turn is ever taken. The table is in spec 149; the two that decided the
shape:
...
that costs a turn against a live model, so it was not measured. Spec 149 records the alternatives and
