...
agents. One commit, `ce076f3`, sitting on Session 166's own commit — so the merge base was already
in main and every Rust file auto-merged. What it cost was the two things the layout charges any
branch written before it:

**A number is only free with respect to the tree in front of it** — the third time in two days.
Their spec kept **148**, which was free; their feature row kept **F222**, which was free; their
session entry was numbered 163 and **became 167**, because main had grown four entries past the one
they branched from. Their parent spec moved under them: every `spec 146` in the branch now reads
**149**, in the row, the spec, the evidence document and the log entry.

**An action declared the old way does not merge into contract 9 — it converts.** Git knew
`orchestrator/actions/grant-session-message.sh` belonged somewhere under the renamed directory and
said so; what it could not know is that contract 9 declares an action **by name**:

| Their shape (contract 8) | Merged shape (contract 9) |
| --- | --- |
| `"script": "orchestrator/actions/grant-session-message.sh"` | `"action": "grant-session-message"` |
| `orchestrator/actions/grant-session-message.sh` | `actions/posix/grant-session-message.sh` |
| *(nothing)* | `actions/win/grant-session-message.ps1`, written here |

The `.ps1` is the ninth port. It parses under Windows PowerShell 5.1.26100 on `pr0fe@192.168.31.217`
through `Parser::ParseFile`, like the other eight, and it has run no more than they have. F223's
criteria say "eight" and stay as accepted — the count is the owner's to correct, and the row now
records that the ninth exists and is subject to the same gate.

**Their suite caught the move, which is the point of it.** Seven of the eight cases failed on
`spawn /Users/alex/red/target/debug/red-launch ENOENT` — a `'../../red/target'` written from
`orchestrator/tests/`, one level too high from `tests/`. The eighth failed differently and more
usefully: *"a confirm nobody can answer is refused, not bypassed"*, because it drives the action
script itself at `'../actions/grant-session-message.sh'`, which is not where a contract 9 action
lives. Both are the same defect the layout change plants in any file that moved with a relative path
in it, and the assertion that found the second one is the one a blind path-sed would have left red.

Gates: `./init.sh`, `design.py check`, `agent_names.py check`, `features.py validate` — green.
**379/379 JS tests, 380/380 Rust tests.** The graph was regenerated rather than hand-merged, as
before.

Remaining, and not this merge's: F222 stays `passes: false` on its own terms — nothing in it has run
against a live CLI, and the row says which measurement is missing.
## Session 167 (macos) — 2026-09-16 — one line said to a pane that is already running (F222, spec 148)

*Written on the `session-message` line, one commit past Session 166, and merged into main on
2026-09-17 once the workspace it was written in paused its agents. It was numbered 163 there and
takes the next free number here; its own spec keeps 148, which was free, and its parent spec is 149
rather than 146.*
...
back — spec 149's handshake, attached to a pane that is already up. Six controls, and the fourth is
...
`SeedWatch::listening` takes the last-output time from the caller — spec 149 decision 9's own rule,
applied.
...
pane in the spec is its own fake TUI. kimi's composer echo was measured by spec 149 against 0.42.0 at
...
safe. `agents/mcp-worker.mjs` serves **38 tools**, and their descriptions are not documentation —
they are the surface. `tools/list` returns them verbatim and an agent reads them to decide what to
call, so "identical to the JS worker's" means byte-identical prose. Fifteen kilobytes of description
retyped into Rust is a transcription error waiting to happen, and nobody would notice a dropped
clause until a CLI made a wrong call because of it.

So it is **captured, not retyped**: a client connected to the running worker, asked for
`tools/list`, and the answer — with the server identity, the instructions and the capabilities —
became `red/red-mcp/src/tools.json`. red-mcp serves exactly that.

What keeps it honest while both exist: the spec starts **both** servers against one live workspace,
drives them with the same MCP client, and compares the list in order, every description, schema and
annotation, the identity, the instructions and the capabilities. That is a live two-implementation
comparison today; when F185 deletes the worker it becomes the frozen record, which is where F148,
F172 and F178 all ended up.

Sabotages: a description drifting by three words, and the JS worker growing a tool (`list_moons`)
without a recapture — the second is the one the row is for, because it is what leaves the Rust side
a version behind in silence.

red-mcp checks its binding at startup the way the worker does (loopback host, 64-hex token, the
instance the context names, the root still there), so a pane whose workspace moved is told at once
rather than on its first tool call. `tools/call` is refused by name with `-32601`: a server that
answered a call plausibly without being able to make it happen would be worse than one that says it
cannot. `red_core::http` now holds the one authenticated GET both red-link and red-mcp make.

Commands: `npm test` (**316 of 316**), `cargo test -p red-mcp`, `python3 tools/design.py check`
(with `CMakeLists.txt` regenerated by cmkr from the edited `cmake.toml`), `python3
tools/features.py validate` (137 features).

**F184 passes.** F185 carries `tools/call` for the full surface, the native exec change and the
deletion of `mcp-worker.mjs` and `runtime/tools.mjs`. F186 is the owner's: a real pane of each CLI
completing a task-scoped action through the Rust MCP is a dogfooding run, not a fixture, and
spawning panes to manufacture it would break the delegation boundary.
