...
async fn spawn_pane(front: &Arc<Front>, options: &Value) -> Result<Value, String> {
    ...
    if kind == "agent" {
        ...
        /* A brief for a CLI that takes none on its command line, to be TYPED into the pane by the
           service that holds its master side (F221, spec 146). The line names a file rather than
...
pub(crate) fn pane_snapshot(session: &serde_json::Value) -> serde_json::Value {
    ...
    /* Whether the brief this pane was seeded with actually arrived (F221, spec 146). Only a seeded
