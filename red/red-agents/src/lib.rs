...
/* How a CLI is handed the brief a spawn carries (F221, spec 146): `argv` is the last positional
...
fn declared_since(raw: &Value, view: &mut serde_json::Value) {
    ...
    /* F221: how this CLI is handed the brief a spawn carries. A kind, never a name — `argv` and
       `paste` are the two rEngine implements (spec 146). */
...
mod tests {
    ...
    /* F221, spec 146. The live failure this guards: a brief appended to a command line reached a CLI
