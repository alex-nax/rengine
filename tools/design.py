...
def product_module(tokens):
    ...
    return "\n".join([
        ...
        "export const PRODUCT_SUITE = %s;" % json.dumps(product["suite"]),
    ]) + "\n"
...
def product_literals(tokens, targets=None):
    ...
    working in."""
    product = tokens["product"]
    names = [product["name"]] + list(product.get("retired", []))
...
def theme_sources(tokens, layers, presets):
    ...
    h = head + ["#ifndef RENGINE_THEME_H", "#define RENGINE_THEME_H", "#include <stdint.h>", '#include "microui.h"',
                ...
                "#define RE_PRODUCT_SUITE %s" % json.dumps(tokens["product"]["suite"]),
...
def product(target=None):
    ...
    print(json.dumps({"name": tokens["product"]["name"], "suite": tokens["product"]["suite"],
