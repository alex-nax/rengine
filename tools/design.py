...
"""Bridge the desktop's theme source and the Claude Design system project.
  ...
  generate            write editor/theme.h from editor/theme.json, red-core's
                      theme.rs beside it (the Rust target, spec 129 F146), refresh the
...
from design_rust import theme_module as rust_theme_module
...
PRODUCT_MJS = ROOT / "tests" / "product.mjs"
...
def product_module(tokens):
    ...
    return "\n".join([
        ...
        " * editor/theme.json. Edit the declaration, never this file (charter D41, spec 108). */",
...
# The roots this check scans. A directory listed here that does not exist scans nothing
# and says so to nobody, so this list moves with the tree (charter D71, spec 147).
PRODUCT_ROOTS = ("editor", "adapters", "tools", "scripts", "red", "tests")
...
PRODUCT_ALLOWED = ("editor/theme.h", "tests/product.mjs",
                   "red/red-core/src/theme.rs",
                   "tests/product-name.test.mjs")
    ...
    for root in roots:
        for path in ([root] if root.is_file() else sorted(root.rglob("*"))):
            ...
            hint = ("use RE_PRODUCT_NAME from theme.h" if path.suffix in (".c", ".h", ".m")
                    ...
                    else "import PRODUCT_NAME from tests/product.mjs")
...
def theme_sources(tokens, layers, presets):
    ...
    The third return value is red-core's theme.rs (spec 129, F146): the same declaration, so the
    Rust side reads the product name and the theme tokens instead of restating them."""
    ...
    rust = rust_theme_module(tokens["product"], tokens["typography"], tokens["metrics"], metrics,
                             fields, ["--" + tokens["colors"][name]["token"] for name in fields],
                             names, tables)
    ...
    head = ["/* Generated by `python3 tools/design.py generate` from design/tokens.css and the bindings in",
            " * editor/theme.json. Edit the tokens or the bindings, never this file. */"]
...
def theme_header(tokens):
    out = ["/* Generated by `python3 tools/design.py generate` from editor/theme.json. Edit the tokens, not this file. */",
...
def syntax_header(doc, layers, presets):
    ...
    out = ["/* Generated by `python3 tools/design.py generate` from editor/syntax.json and the",
...
def icons_header(font, icons):
    ...
    out = ["/* Generated by `python3 tools/design.py generate` from editor/icons.json. Edit the mapping, not this file. */",
...
def card_reference(layers, presets):
    ...
    names = preset_names(presets)
    out = {"version": 1, "note": "Generated by `python3 tools/design.py cards`. tests/native-design.spec.mjs"
...
def native_build_files():
    ...
    if (NATIVE / "CMakeLists.txt").exists():
        problems.append("editor/CMakeLists.txt is obsolete; targets live in the root cmake.toml")
    return problems
...
def generate():
    ...
    header, source, rust = theme_sources(tokens, layers, presets)
    THEME_H.write_text(header, encoding="utf-8")
    THEME_C.write_text(source, encoding="utf-8")
    THEME_RS.write_text(rust, encoding="utf-8")
    ...
    for problem in problems:
        print("ERROR: " + problem)
    print("Generated theme.h and theme.c, runtime/product.mjs, red-core/src/theme.rs, render/icons.h (%d icons), render/syntax_theme.h (%d schemes), the tokens.json mirror, cards.json and manifest.json for %d cards (%d presets: default, %s)."
...
def product(target=None):
    ...
    print(json.dumps({"name": tokens["product"]["name"], "family": tokens["product"]["family"],
                      ...
                      "generated": ["editor/theme.h", "tests/product.mjs",
                                    "red/red-core/src/theme.rs"],
