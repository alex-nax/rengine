...
def theme_sources(tokens, layers, presets):
    ...
    c += ["};", "const char *const re_theme_preset_names[RE_PRESET_COUNT] = {%s};" % ", ".join('"%s"' % n for n in names), "",
          ...
          "    if (name && !strcmp(name, re_theme_preset_names[i])) { re_theme = re_theme_presets[i]; re_theme_preset = i; return i; }",
          "  }", "  return -1;", "}", "",
...
def generate():
    ...
    for problem in problems:
        print("ERROR: " + problem)
    print("Generated theme.h and theme.c, render/icons.h (%d icons), render/syntax_theme.h (%d schemes), the tokens.json mirror, cards.json and manifest.json for %d cards (%d presets: default, %s)."
