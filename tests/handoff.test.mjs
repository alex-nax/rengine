...
import { readHandoff } from './handoff-index.mjs';
import { conversationHandoff, handoffCapableAgents, forgetRecipes } from './agents-client.mjs';
import { built } from './cargo.mjs';

const ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..');

/* This spec drives a Rust binary, so it builds one first: run alone it would otherwise judge
   whatever binary happened to be on disk (tests/cargo.mjs). */
before(() => built('--bins'));
