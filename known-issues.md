...
| --- | --- | --- |
| KI-001 | Owner selected library-base value as the primary direction; first library proof and meaning of “both streams” remain open. | Refine library ownership/quality and pilot scope; shared command tooling is supporting work. |
| KI-002 | No reviewed feature inventory yet. | Present the concrete proposal before creating `features.json`. |
| KI-003 | “Powered by rEngine” has no agreed minimum adoption contract. | Resolve selective adoption versus a required harness/capability baseline. |
...
| KI-009 | Orchestrator scope is macOS/Windows, adapters first, basic editing with optional Vim mode, and Quest 2D-first; sessions detach into a browser and desktop v0 includes a live flat-game tab; toolkit and detailed budgets remain open. | Continue from `docs/specs/002-orchestrator.md`; test terminal/editor/game surfaces on both desktop platforms before committing to a toolkit. |
