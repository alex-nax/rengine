...
`orchestrator/native/theme.json`, and generated by `python3 tools/design.py generate` into
`orchestrator/native/theme.h` (`RE_PRODUCT_NAME`, and `RE_DEFAULT_TITLE` aliased to it) and
`orchestrator/runtime/product.mjs` (`PRODUCT_NAME`), and — the same declaration's Rust target
