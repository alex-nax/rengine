...
or listed in `tests/suite-coverage.test.mjs` with its reason; a fixture that
...
Build definitions live in `cmake.toml` (desktop) and `adapters/sdl2/cmake.toml` (surface adapter).
Every `CMakeLists.txt` is generated by cmkr, pinned at `v0.2.46` through `cmake/cmkr.cmake`, which
bootstraps cmkr into the build directory on first configure, and the generated files are committed.
Edit the TOML, never the generated file; `python3 tools/design.py check` rejects hand-edited output.

The product is **Red**; the umbrella is **Red Suite** (charter D41). rEngine is the project that
builds it, and keeps its own name. Red's name is declared once, in the `product` block of
`editor/theme.json`, and generated by `python3 tools/design.py generate` into
`editor/theme.h` (`RE_PRODUCT_NAME`, and `RE_DEFAULT_TITLE` aliased to it) and
`tests/product.mjs` (`PRODUCT_NAME`), and — the same declaration's Rust target
(spec 129, F146) — `red/red-core/src/theme.rs` (`PRODUCT_NAME`/`PRODUCT_FAMILY` and the theme
constants). Never type the product's name into shipping
