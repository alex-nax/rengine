/* F161 (spec 102, spec 133): the record the IDE bridge is judged against.
 *
 * Red as a Claude Code IDE is a set of distrust rules read out of one CLI binary and checked against
 * the running CLI: which pid a lock may name, whose stale locks may be collected, what a connection
 * must present, what is answered and — as importantly — what is not. This replays the corpus against
 * the live module, so the record cannot drift from what it froze, and then reads the rules back out
 * of the record so a regeneration that lost one is a red test rather than a quiet edit.
 */
import test from 'node:test';
import assert from 'node:assert/strict';
import { CASES, RECORDED, answers, jsHarness } from './ide-corpus.mjs';

test('the recorded bridge answers are the ones ide.mjs gives', { timeout: 300000 }, async () => {
  assert.ok(RECORDED, 'ide-corpus.json is present; it is the evidence, not a cache');
