...
export async function nativeClient(instance, { root = '', terminal = '', agent = '', game = '' } = {}) {
  const executable = process.env.RENGINE_NATIVE_BINARY ?? path.resolve('.cache/desktop/bin', process.platform === 'win32' ? 'rengine.exe' : 'rengine');
  ...
  return { child, command, until, click, key, close, diagnostics: () => diagnostics };
}
