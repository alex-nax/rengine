...
test('contract 6 carries tracker.write and the agent menu, and a contract-5 declaration is accepted unchanged', async t => {
  ...
  // it reaches neither tracker.write nor agents, and the assertions below ran unchanged.
  assert.equal(CONTRACTS.at(-1), 11, 'the ceiling moved with the keys');
...
/* F221, spec 146. The live failure: a spawn appended the rendered brief to the pane's argv, which is
