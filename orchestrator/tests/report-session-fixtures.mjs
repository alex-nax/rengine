 ...
 * orchestrator/tests/report-session-fixtures.json. Like store-corpus.mjs, this is a tool
...
/* The capture itself, run as a tool: node orchestrator/tests/report-session-fixtures.mjs */
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  const stub = await stubHost();
  const out = {};
  for (const kase of CASES) out[kase.name] = await runCase(['node', REPORTER], kase, stub);
  await stub.close();
  const file = path.join(ROOT, 'orchestrator/tests/report-session-fixtures.json');
