...
test('every registry dependency is pinned by version and checksum in a committed Cargo.lock', async () => {
  ...
  /* The workspace's own crates, read from the manifest rather than listed here: a member added by a
     row that forgot this file would otherwise be reported as an unpinned registry dependency, which
     is a true statement about the wrong thing. */
  const members = [...(await read('red/Cargo.toml')).matchAll(/"(red-[a-z-]+)"/g)].map(([, name]) => name);
  assert.ok(members.length >= 7, `the workspace names its members: ${members.join(', ')}`);
  const local = new Set(members);
