...
test('every registry dependency is pinned by version and checksum in a committed Cargo.lock', async () => {
  ...
  const local = new Set(['red-core', 'red-link', 'red-agents', 'red-store', 'red-pty', 'red-mcp', 'red-host']); // F139's skeletons, F148a's registry, F169's store, F176's pty, F184's MCP server, F188's front door
