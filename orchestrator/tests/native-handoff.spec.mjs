...
`, { mode: 0o700 });
    ...
    const oldDesktop = desktops[0].id;
    const requested = await mcp.callTool({ name: 'reload_desktop', arguments: { id: oldDesktop } });
    assert.ok(!requested.isError, JSON.stringify(requested)); assert.equal(JSON.parse(requested.content[0].text).status, 'accepted');
    for (let i = 0; i < 150 && (gui.diagnostics().match(/Rebuilding rEngine/g) ?? []).length < 2; i++) await delay(20);
    assert.equal((gui.diagnostics().match(/Rebuilding rEngine/g) ?? []).length, 2);
    await delay(1200);
    await gui.until(s => s.connected && s.tabs.some(t => t?.type === 2 && t.dirty && t.text?.includes('reload draft')), 'MCP reload recovers dirty draft');
    const replaced = JSON.parse((await mcp.callTool({ name: 'list_desktops', arguments: {} })).content[0].text).desktops;
    assert.equal(replaced.length, 1); assert.notEqual(replaced[0].id, oldDesktop);
    assert.equal(server.sessions.snapshot(agent.id).pid, agent.pid);
    assert.equal((await readFile(marker, 'utf8')).trim().split('\n').length, 1);
    const stale = await mcp.callTool({ name: 'reload_desktop', arguments: { id: oldDesktop } }); assert.equal(stale.isError, true);
