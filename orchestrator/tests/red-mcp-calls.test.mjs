/* F185 (F150b, spec 129, KI-100): red-mcp answers tool CALLS as the JS worker answers them.
 *
...
function normalise(value, directory) {
...
async function converse(t, command, args, session, marker) {
...
test('red-mcp answers every tool call the way the JS worker answers it', { timeout: 600000 }, async t => {
  ...
  const js = await workspace(t, 'js');
  const rust = await workspace(t, 'rust');
  /* The other agent is the same identity in both workspaces, so the two conversations differ in
     nothing but which server answered them. */
  const other = { ...identity('the-other-agent'), agentId: '11111111-2222-3333-4444-555555555555' };
  const claim = where => async () => { await ok(where.worker, 'token-action', { rootId: where.root.id, action: 'contest', reason: 'the other agent takes it' }, other); };
  const fromJs = await converse(t, process.execPath, [path.join(ROOT, 'orchestrator/agents/mcp-worker.mjs'), '--context', js.contextFile], js.session.id, claim(js));
  const fromRust = await converse(t, BINARY, ['--context', rust.contextFile], rust.session.id, claim(rust));

  if (process.env.RENGINE_SHOW_CONVERSATION) {
    for (const [index, [name]] of CALLS.entries()) {
      const answer = fromJs[index];
      console.error(`${name}: ${JSON.stringify(answer).slice(0, 160)}`);
    }
  }
  const differences = [];
  for (const [index, [name, args]] of CALLS.entries()) {
    const expected = normalise(fromJs[index], js.directory);
    const actual = normalise(fromRust[index], rust.directory);
    try { assert.deepEqual(actual, expected); }
    catch { differences.push({ name, args, expected, actual }); }
  }
  assert.deepEqual(differences.map(d => d.name), [],
    `every call answers the same:\n${differences.map(d => `${d.name}\n  js:   ${JSON.stringify(d.expected).slice(0, 400)}\n  rust: ${JSON.stringify(d.actual).slice(0, 400)}`).join('\n')}`);

  /* A conversation that refused everything would compare two servers that did nothing. */
  const refused = fromJs.filter(answer => answer.isError).length;
