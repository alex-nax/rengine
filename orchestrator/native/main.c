...
int main(int argc, char **argv) {
  ...
  for (int i = 1; i < argc; i++) {
    ...
    else if (!strcmp(argv[i], "--help")) { puts("rengine [--connection sidecar.json] [--font FILE] [--renderer opengl|metal|vulkan|sdl|seam] [--smoke-test --snapshot FILE.bmp] [--automation]\nAutomation accepts local stdin test events only when explicitly enabled."); return 0; }
  ...
  if (!backend) { fprintf(stderr, "Unknown renderer '%s'; use opengl, metal, vulkan, sdl or seam.\n", renderer ? renderer : ""); return 2; }
  SDL_SetMainReady();
