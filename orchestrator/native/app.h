...
typedef struct { int id, operation, tab, generation, revision; char root[65], game[65]; long timeout; } RePending;
...
typedef struct ReApp {
  ...
  cJSON *state, *previous_layout, *controls, *formats, *dashboards, *dashboards_opened, *game_configs;
  char root[65], initial_terminal[65], initial_agent[65], initial_game[65], menu_root[65];
  char project_input[1024], agent[256], status[512];
  mu_Rect menu_rect; int menu_x; bool menu_raise;
...
const cJSON *re_app_games(ReApp *app, const char *root);
bool re_app_game_entry(ReApp *app, const char *root, const cJSON *game, char *label, size_t size);
void re_app_launch_game(ReApp *app, const char *root, const char *id);
void re_app_games_probe(ReApp *app, const char *root, bool refresh);
