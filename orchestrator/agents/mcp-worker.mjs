...
const tool = (name, description, inputSchema, readOnlyHint, action) => server.registerTool(name, {
  description, inputSchema, annotations: { readOnlyHint, destructiveHint: ['stop_session', 'open_script'].includes(name), openWorldHint: ['open_script', 'preview_file', 'dashboard_capture'].includes(name) },
}, async values => {
  let state;
  try {
