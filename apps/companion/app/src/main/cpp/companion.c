...
static void draw_frame(int width, int height) {
  ...
  mu_begin(&ui);
  if (mu_begin_window_ex(&ui, "rEngine companion", mu_rect(0, 0, logical_w, logical_h),
    ...
    mu_layout_row(&ui, 1, (int[]){-1}, 0);
    if (mu_button(&ui, "Sessions")) taps++;
    if (mu_button(&ui, "Tasks")) taps++;
    if (mu_button(&ui, "Approve")) taps++;
    char counted[64];
    snprintf(counted, sizeof(counted), "%d tap(s)", taps);
    mu_label(&ui, counted);
    mu_end_window(&ui);
  }
  mu_end(&ui);
...
static void on_command(struct android_app *app, int32_t command) {
  if (command == APP_CMD_INIT_WINDOW && app->window && !backend) {
    ...
    backend = re_opengl_backend_seam_open(app->window, fonts);
    ...
    /* The desktop's palette, metrics and spacing, from the same generated theme (D33/D41). Not the
       owned control layer yet -- see KI-089 -- but the same colours and sizes it draws with. */
    re_theme_apply(ui.style);
    ui.style->size.y = RE_THEME_LINE_HEIGHT;
